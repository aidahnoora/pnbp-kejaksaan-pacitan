    ...
    <div class="content-wrapper">
        <div class="row">
            <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Laporan Bidang {{ ucfirst($bidang) }}</h4>

                        <form action="{{ route('laporan') }}" method="GET">
                            <div class="row mb-3">
                                <div class="col">
                                    <select name="bidang" class="form-control form-control-sm" required>
                                        <option value="" selected disabled>Pilih Bidang</option>
                                        @foreach ($bidangs as $item)
                                            <option value="{{ $item->alias }}">{{ $item->nama_bidang }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col">
                                ...
                                <div class="col">
                                    ...
                                    <a href="{{ route('laporan.exportPdf', ['bidang' => $bidang, 'bulan' => $bulan ?? '', 'tahun' => $tahun ?? '']) }}"
